...
This capability defines how EmployeeK models its relational schema on top of the local Postgres 16 provided by `db-local-postgres`. It establishes Prisma 7 as the sole ORM, the location and structure of the schema and its migrations, the typed client surface the rest of `apps/api` consumes, the seed data that populates reference tables, and the developer scripts and Turbo wiring that make the whole thing one-command reproducible. Downstream changes (CH4 Fastify, CH6 OpenAPI, CH7 CI) consume this capability as a stable contract.

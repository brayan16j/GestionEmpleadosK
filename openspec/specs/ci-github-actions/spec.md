...
Defines the GitHub Actions continuous integration pipeline that gates merges into `main`. The pipeline enforces code quality (format, lint, types, build), runs the full test suite against a Postgres service container, prevents OpenAPI contract drift, validates OpenSpec change artifacts, and (on pull requests) checks Conventional Commit messages. Promoted from `setup-ci-github-actions`.

# Local Postgres Database Specification

## Purpose

This capability defines how EmployeeK developers run a local PostgreSQL database for day-to-day development. It standardizes a Docker Compose-based Postgres 16 service, its persistence and healthcheck behaviour, the `.env`-driven configuration contract, the root-level lifecycle scripts (`db:up`, `db:down`, `db:logs`, `db:reset`), and the developer documentation that explains the workflow — all while leaving the quarantined legacy stack untouched.
